...
def get_ai_message(question):
    ...
    # 이미지 + 공지사항만 존재하는 경우.
    if final_image != "No content" and final_text == "No content" and final_score > 1.8:
        # 이미지 URL 조인
        if isinstance(final_image, list):
            image_urls = "\n".join(final_image)
        else:
            image_urls = final_image
    ...
    # 이미지 + LLM 답변이 있는 경우.
        ...
        if final_image and final_image != "No content":
            if isinstance(final_image, list):
                image_urls = "\n".join(final_image)
            else:
                image_urls = final_image
        ...
        if not qa_chain or not relevant_docs:
            if final_image != "No content" and final_score > 1.8:
                data = {
