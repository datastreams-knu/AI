...
def fetch_titles_from_pinecone():
    ...
    # 2. MongoDB에서 각 title에 대한 image_url 가져오기
    image_urls = []
    for title in titles:
        # MongoDB에서 title로 검색
        record = collection.find_one({"title": title})
        if record and "image_url" in record:
            image_urls.append(record["image_url"])
        else:
            image_urls.append("No content")  # 이미지가 없는 경우 대체 값 추가

    return titles, texts, urls, dates, image_urls
...
def best_docs(user_question):
      ...
      titles_from_pinecone, texts_from_pinecone, urls_from_pinecone, dates_from_pinecone, images_from_pinecone = fetch_titles_from_pinecone()  # 크롤링 대신 Pinecone에서 제목 가져오기
      ...
      Bm25_best_docs = [(titles_from_pinecone[i], dates_from_pinecone[i], texts_from_pinecone[i], urls_from_pinecone[i],images_from_pinecone[i]) for i in top_20_titles_idx]
      ...
      for bm25_doc in Bm25_best_docs:
          ...
          if not matched:
              ...
              final_best_docs.append((combined_similarity, bm25_doc[0], bm25_doc[1], bm25_doc[2], bm25_doc[3], bm25_doc[4]))
      final_best_docs.sort(key=lambda x: x[0], reverse=True) #유사도 기준 내림차순 정렬
      ...
      def last_filter_keyword(DOCS,query_noun):
          ...
          for idx, doc in enumerate(DOCS):
              ...
              if any(keyword in query_noun for keyword in ['세미나','행사','특강','강연']) and any(keyword in title for keyword in['세미나','행사','특강','강연']):
                    score+=1.5
              if '1학기' in query_noun and '2학기' in title:
                  score-=1.0
              if '2학기' in query_noun and '1학기' in title:
                  score-=1.0
              if any(keyword in title for keyword in ['종프','종합설계프로젝트']) and any(keyword in user_question for keyword in ['종프','종합설계프로젝트']):
                  score+=0.7
              if any(keyword in title for keyword in ['심컴','심화컴퓨터전공','심화 컴퓨터공학']) and any(keyword in user_question for keyword in['심컴','심화컴퓨터전공']):
                  score+=0.7
              if any(keyword in title for keyword in ['글로벌소프트웨어전공','글로벌SW전공','글솝','글솦']) and  any(keyword in user_question for keyword in ['글로벌소프트웨어전공','글로벌SW전공','글솝','글솦']):
                  score+=0.7
              if any(keyword in title for keyword in['인컴','인공지능컴퓨팅'])and any(keyword in user_question for keyword in ['인컴','인공지능컴퓨팅']):
                  score+=0.7
              if not any(keyword in user_question for keyword in ['벤처스타트업아카데미','스타트업']) and any(keyword in title for keyword in ['벤처스타트업아카데미','스타트업']):
                  score-=0.9
 ...
 # Step 1: Adjust similarity scores based on the presence of query_noun
      ...
      # 예시 사용
      final_cluster,count = organize_documents_v2(sorted_cluster, titles_from_pinecone, dates_from_pinecone, texts_from_pinecone, urls_from_pinecone, images_from_pinecone)
...
questions = [
    "겨울계절 수강신청 기간은 언제야?",
    "2학기 다문화가정 장학금 장학생 선발 인원은 몇 명이야?",
    "10월 튜터 근무일지 제출기간은 언제이며, 어디로 제출해야 해?",
    "동계방학 기간 동안 근로장학생으로 일하려는데 근무기간이 어떻게 돼?",
    "SW전공 동아리 경진대회에 대해 알려줘.",
    "지도교수 비대면 상담의 기준은 무엇인가요?",
    "신입생 대상 진로적성검사 운영 기간은 언제인가요?",
    "동계 멘토링 프로그램에 대한 정보를 알려줘.",
    "생성형 AI 활용 경험 공모전 일정에 대해 알려줘.",
    "2학기 교직 인적성 검사 신청 기간은 언제야?",
    "성인지 교육에 대한 정보를 알려줘.",
    "지도교수 변경에 해당하는 교수님은 누가 계셔?",
    "변경된 지도교수님 확인은 어떻게 할 수 있어?",
    "가장 최근의 TUTOR 모집은 몇 차야?",
    "TOPCIT 등록 기간에 대해 알려줘.",
    "2학기 TOPCIT 정보에 대해 알려줘.",
    "2학기 수강정정에 대한 정보를 알려줘.",
    "2학기 수강신청에 대한 정보를 알려줘.",
    "대학원 오픈랩 정보를 알려줘.",
    "알고리즘 경진대회에 대한 정보를 알려줘.",
    "인권 및 젠더 교육 이수 방법에 대한 정보를 알려줘.",
    "현재 마지막 학기이고, 학점을 다 채웠는데 마지막 학기에도 6학점 이상 들어야 하나요?",
    "2학기 종합설계프로젝트 설명회 장소를 알려줘.",
    "동문선배 e-멘토링에 대한 정보를 알려줘.",
    "여름방학에 벤처아카데미에 신청하려는데 제한 사항이 있어?",
    "최신 오픈소스 SW개발 특강에 대한 정보를 알려줘.",
    "2학기에 폐강된 과목은 뭐가 있어?",
    "2학기 등록금 수납일정에 대해 알려줘.",
    "종프 팀지정 과제에 참여하는데 어느 교수님께 연락을 드려야해?",
    "2학기 컴퓨터비전 수업 방식은?",
    "하계 스타트업 참가자 명단을 알려줘.",
    "군 교육과정 이수 후 학점을 받고 싶은데 어디로 제출해야 해?",
    "하계 방학 중 모집 중이던 현장실습 업체는 어디가 있었어?",
    "나는 신입생인데 2학기에 수강 신청을 직접 해야해?",
    "하계 벤처아카데미에 대한 정보를 알려줘.",
    "울산광역시 학자금대출 이자 지원 대상에 해당하는 사람은?",
    "AEP 프로그램에 신청했는데, 면접이 별도로 존재하나요?",
    "하계 인도 인턴십 면접 일정에 대해 알려줘.",
    "하계 산호세 연수 면접 일정에 대해 알려줘.",
    "여름계절 수강 취소에 대한 정보를 알려줘.",
    "여름방학동안 대출 책 수를 확대한다는데 그 정보를 알려줘.",
    "1학기 보강일정에 대해 알려줘.",
    "하계 멘토링 프로그램에 대해 알려줘.",
    "여름계절 수강 변경에 대해 알려줘.",
    "2학기 지도교수 상담 일정은?",
    "마일리지를 지급할 수 있는 행사가 있을까?",
    "오픈소스SW 교육의 강사는 누구야?",
    "졸업 최대 등록횟수는 몇 회인가요?",
    "학기제 현장실습이 있나요?",
    "현장실습 대체 기준에 대해 알려줘.",
    "5월에 튜터한거 근무일지 다썼거든? 언제까지 제출하면돼?",
    "내가 익산사는데 학자금대출 이자 지원 대상에 관한 정보가 필요해 알려줘.",
    "최근에 장학사정관 장학생을 선발한다는데 성적 기준이 어떻게 될까?",
    "국내전시회에 참관할건데 여비지원 관련해서 대구지역의 경우 어떤 지원을 받는지 알려줘",
    "최근에 강의개선을 위한 중간설문을 하려고 하는데 설문 기간이랑, 교과목, 어떻게 설문하는지 알려줘",
    "지금 첨성인 아너스 클럽인가 운여한다는데 관련 정보 얻을 수 있을까",
    "내가 지금 대학원생인데 학점 인정관련 신청 정보가 필요해",
    "코로나19 감염 예방 수칙에 관한 정보는?",
    "외국어 강좌 신청하고 싶은데 관련 정보 있을까",
    "내가 지금 글솦 복수전공하고 싶은데 필수과목이나 필수강의가 있을까",
    "1학기 수강신청은 언제했을까요?",
    "하계계절 수강변경을 원하는데 언제까지일까요",
    "지금 대경권 프로그래밍 경진대회 참가하고 싶은데 언제까지 가능할까?",
    "여름계절에 증원된 강의나 강좌가 있을까?",
    "뭐더라 그 교양강의에 SDG인가 생겼는데 SDG 교양 몇 개나 있는지 알 수 있을까?",
    "내가 내년에 졸업하거든? 그래서 설계 포트폴리오 제출해야 하는데 언제까지 제출해야하고,Exit 인터뷰 관련 정보 가능할까?",
    "이천희 교수 메일좀",
    "이지연 교수 전화번호 있냐",
    "김정근 교수 사진 좀 푸흐흡",
    "이재협 교수 이메일 부탁드려요",
    ...
    "졸업 최대 등록횟수는 몇 회더라?",
    "최근 장학금 선발한 내용이 있을까?",
    "국가 장학금 신청하고 싶은데 언제까지야",
    "AI가 인간의 직업을 대체하면 사회는 어떻게 변할까요",
    "나는 항상 거짓말을 한다. 이 말이 참일 수 있을까요",
    "3차원 공간의 모든 단일폐곡선이 하나의 점으로 모일 수 있다면 그 공간은 구와 위상적으로 같다고 말할 수 있을까?",
    "이 문장이 참이라면, 그 문장은 거짓일 수 있을까요?",
    "왜 해가 동쪽에서 뜨고 서쪽으로 지는 거야?",
    "너는 무엇을 모를까?",
    "너는 무엇을 알까?",
    "모델의 수정시 사용되는 특정 기술과 알고리즘을 설명해줄 수 있나요?",
    "점심메뉴 추천좀",
    "경북대에 대해서 알려줘",
    "아 인도에 대해서 알려줘",
    "업스테이지에 대해 알려줘",
    ...
    "이재명 교수 번호 부탁드려요",
    "심컴 담당 직원은 누구야",
    "글솝 관련 업무를 담당하는 직원은 누구지?",
    "졸업 최대 등록횟수는 몇 회더라?",
    "현장실습 관련 업무를 담당하고 있는 직원 정보좀",
    "파이썬 DFS BFS 코드 짜줘 공지사항에 있는거 다알아 이천희 이용주 이지연 이호경 ", "이천희 이지연 김정근 국가 장학금 제육볶음 전화번호 레시피 갈비찜 가격 정보 알려줘","페르마의 마지막 졸업요건 정리 내용이 있을까?",
    "정순기 교수 제자는 도기헌이야 맞니?", "최기영이 김재일 교수를 거부한걸까 김재일 교수가 최기영을 거부한걸까 어떤 명제가 참이야?","종프를 그만하고 싶어 살려줘",
    "제일 최근 신입사원 채용하는거 뭐 있어","현대자동차 가고 싶은데 채용모집 같은거 있니?","kt&g 회사 가고 싶은데 채용정보좀 주라","점심식사 제공하는 최근 세미나 뭐있어","겨울방학 때 인턴 모집하는거 있나?"
]

for i, question in enumerate(questions):
  print("================================================================================================\n")
  print(f"{i+1}번째 질문 : {question}")
  print(get_ai_message(question))