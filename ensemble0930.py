...
# 프롬프트 템플릿 정의
prompt_template = """너는 경북대학교 컴퓨터학과 사무실 직원이고, 공지사항의 내용을 정확히 전달할 의무가 있습니다.
...
5. 정확한 정보만을 제공하고, 불확실한 경우 그냥 모른다고 대답해주세요.
...
def get_ai_message(user_question):
    ...
    qa_chain=get_answer_from_chain(best_docs)
    ai_message = qa_chain.invoke({"question": user_question})

# 상위 3개의 참조한 문서의 URL 포함 형식으로 반환
    doc_references = "\n\n".join([f"참고 문서 URL: {doc[1]}" for doc in best_docs[:3] if doc[1] != 'No URL'])  # 상위 3개 문서만 선택
