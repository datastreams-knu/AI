...
def extract_text_and_date_from_url(urls):
    ...
    def fetch_text_and_date(url):
        try:
            ...
            text = "\n".join([para.get_text() for para in paragraphs]) if paragraphs else "Unknown Content"
            ...
            if title != "Unknown Title":
                return title, text, date, url  # 문서 제목, 텍스트, 날짜, URL 반환
            else:
    ...
    # 유효한 데이터만 추가
    all_data = [(title, text, date, url) for title, text, date, url in results if title is not None]
    return all_data
...
doc_urls = []
doc_dates = []
for title, doc, date, url in document_data:
...
def get_best_docs(user_question):
    try:
        # 사용자 질문을 전처리하여 명사만 추출
        okt = Okt()
        nouns = okt.nouns(user_question)  # 명사 추출
        print(nouns)
        ...
        tfidf_best_docs = [(titles[i], doc_dates[i], texts[i], doc_urls[i]) for i in top_tfidf_indices]
        # 결과 출력 (TF-IDF)
        print("TF-IDF Best Documents:")
        ...
        tfidf_count = len(tfidf_best_docs)
        for idx in combined_best_doc_indices:
            if idx < tfidf_count:
                best_docs.append(tfidf_best_docs[idx])
            else:
                pinecone_idx = idx - tfidf_count
                if pinecone_idx < len(pinecone_docs):
                    best_docs.append(pinecone_docs[pinecone_idx])
