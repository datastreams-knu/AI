...
def get_best_docs(user_question):
    try:
        ...
        # 불필요한 부분 제거
        cleaned_nouns = clean_noun(okt.nouns(user_question))
        #print("정제된 명사:", cleaned_nouns)

        # 리스트를 공백으로 구분된 문자열로 변환
        nouns = ' '.join(cleaned_nouns)
  ##############################################################
  ...
  ##############################################################
        ...
        # 결합된 유사도 배열 생성
        combined_similarities = np.concatenate((np.array(filtered_similarities), np.array(pinecone_similarities)))
...
def get_answer_from_chain(best_docs, user_question):
    ...
    doc_image_urls = [doc[4] if len(doc) == 5 else "No content" for doc in best_docs]



    documents = [
        Document(page_content=text, metadata={"title": title, "url": url, "doc_date": datetime.strptime(date, '작성일%y-%m-%d %H:%M')})
        for title, text, url, date ,doc_image_urls in zip(doc_titles, doc_texts, doc_urls, doc_dates,doc_image_urls)
    ]
